...
transformer_file_path = PREPROCCESSING_OBJ_FILE
...
def predict_datapoint():
    if request.method == 'GET':
        ...
        data = CustomData(
            ...
            Vehicle_condition = int(request.form.get('Vehicle_conditon')),
...
def perform_batch_prediction():
    if request.method == 'GET':
        ...
        if file and '.' in file.filename and file.filename.rsplit('.',1)[1].lower() in ALLOWED_EXTENSION:
            ...
            return render_template("batch.html", prediction_result = output, predicton_type='batch')
        else:
